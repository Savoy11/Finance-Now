...
| RP-7 | **Per-provider staking risk dimensions** — the six editorial 1–10 scores (custody, counterparty, contract, slashing, liquidity, regulatory) that D14 had kept as "reference inputs" after removing every composite of them | 2026-09-25 | **Removed** | Owner, mid-way through T-394's editorial pass on them: *"we should consider removing this risk assessment as it may sound like a recommendation"*, then, asked directly: *"Remove them."* RP-6's line — a risk figure on the thing the reader is looking at reads as a recommendation — applied to editorial figures on a provider. The finding that sharpened it: the `/staking` page had never rendered the six; a person met them only through `/api/v1`, the MCP tool and the agent tool, i.e. through an AI agent narrating them, which is where a reference number most easily becomes advice and which no on-page disclaimer reaches. Removed from the catalog (`RiskProfile`, `risks`, `assetRisks`, `mergedRisks`), the API (`riskBreakdown`), OpenAPI, the MCP and agent tools, the discovery route's derived `risks`, and `lib/risk/profiles/stakingAdapter.ts`. `custodyModel` stays — a fact, not a score. Guard: `riskDimensionsRemoved.test.ts`. Record: D26 | A regulatory review concludes that editorial per-provider risk figures are publishable, at which point the six dimensions are recoverable from git history — but so is the question of what a 1–10 with no methodology behind it was ever measuring |
