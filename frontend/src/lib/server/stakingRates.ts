...
// ─── Staking rates: ONE collector, two surfaces ──────────────────────────────
//
// Lived in app/live-data/staking-rates/route.ts until 2026-09-18. It moved here
// because /api/v1/staking/opportunities had grown its OWN copy of this job —
// three upstreams instead of seven, its own hand-derived offsets, its own
// fallback table — and the copy was broken in ways the original was not:
//
//   • its Lido parse multiplied an already-percentage value by 100, got 231.6,
//     failed its own `apr < 30` sanity bound and silently dropped the reading;
//   • since the four exchange keys derive only `if (rates.lido_eth)`, that one
//     failure took rocketpool/coinbase/kraken/binance down with it;
//   • its Jito endpoint (/api/v1/apy) is a 404 — the live one is
//     /api/v1/stake_pool_stats.
//
// Result, measured 2026-09-18: the PUBLIC api served catalog estimates for 6 of
// 7 checked keys while the UI showed live readings — including jito_sol 7.5,
// the exact figure T-397 had corrected to 4.86 in the table below. It fell back
// to `asset.staticApr` in stakingProviders.ts, a second fallback table nobody
// refreshes.
//
// Two implementations of "fetch live, fall back with disclosure" will drift,
// and these did. There is now one, and both surfaces read it.
...
export async function collectStakingRates(): Promise<StakingRatesResponse> {
